...
- ssh into the server
    - For Mac and Linux: `ssh root@primal-genes.com`
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo hrow`
    - replace hrow with your own
- create password
    - `passwd hrow`
- Move .ssh key file
    - `cp -r .ssh /home/hrow`
- change ownership of the .ssh directory
    - `cd /home/hrow`
    - `chown -R hrow:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh hrow@primal-genes.com`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- create a directory for your deployment
    - `sudo mkdir -p /var/www/primal-genes.com/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/primal-genes.com/html`
...
- `sudo vim /etc/nginx/sites-available/primal-genes.com`
...
server {
    ...
    root /var/www/primal-genes.com/html;
    index index.html index.htm index.nginx-debian.html;

    server_name primal-genes.com www.primal-genes.com;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/primal-genes.com /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/hrow/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/hrow/.ssh/github_deploy_key.pub >> /home/hrow/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/hrow/.ssh/github_deploy_key`
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ssh-keyscan -H primal-genes.com >> ~/.ssh/known_hosts 
         scp -r ./dist/* gkephart@primal-genes.com:/var/www/primal-genes.com/html
```
